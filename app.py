...
st.caption(
    "Data note: brand, perfumer, launch year, and note/accord data are real. Ratings, vote "
    "counts, and TikTok view estimates were hand-compiled from a 30-fragrance sample for this "
    "demonstration rather than pulled via a live API or scraper — treat findings as "
...
# ---------------------------------------------------------
# KPIs — computed live from the dataframe, not hardcoded.
# (Previously these were typed-in strings like "Gourmand (166M Avg Views)"
# that didn't actually reflect df. Now they recalculate if the CSV changes.)
# ---------------------------------------------------------
note_cols = [c for c in ['top_notes', 'heart_notes', 'base_notes'] if c in df.columns]
all_notes_flat = []
for col in note_cols:
    for val in df[col].dropna():
        all_notes_flat.extend([n.strip() for n in str(val).split(',')])
top_anchor_notes = [n for n, _ in Counter(all_notes_flat).most_common(2)] if all_notes_flat else ["N/A"]

most_viral_category, most_viral_millions = "N/A", 0
top_rated_category, top_rated_value = "N/A", 0
if 'style_category' in df.columns:
    if 'tiktok_views_est' in df.columns:
        category_views = df.groupby('style_category')['tiktok_views_est'].mean()
        most_viral_category = category_views.idxmax()
        most_viral_millions = category_views.max() / 1e6
    if 'rating' in df.columns:
        category_ratings = df.groupby('style_category')['rating'].mean()
        top_rated_category = category_ratings.idxmax()
        top_rated_value = category_ratings.max()

col1, col2, col3, col4 = st.columns(4)
col1.metric("Database Sample Size", f"{len(df)} Fragrances")
col2.metric("Most Viral Family", f"{most_viral_category} ({most_viral_millions:.0f}M Avg Views)")
col3.metric("Highest Consumer Satisfaction", f"{top_rated_category} ({top_rated_value:.2f}/5)")
col4.metric("Top Formula Anchor Notes", " / ".join(top_anchor_notes))
...
with tab2:
    ...
    if note_cols and all_notes_flat:
        note_counts = Counter(all_notes_flat).most_common(10)
